...
/// This crate has a runtime dependency on artifacts produced by the `simple-program-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages.into_iter().find(|Package { name, .. }| name == "simple-program-ebpf").unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `simple-program-ebpf`, so cargo will automatically rebuild us
        // if `simple-program-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `simple-program-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        cmd.args(["build", "-Z", "build-std=core", "--bins", "--message-format=json", "--release", "--target", &target]);
        ...
        let ebpf_target_dir = out_dir.join("simple-program-ebpf");
        ...
        let status = child.wait().unwrap_or_else(|err| panic!("failed to wait for {cmd:?}: {err}"));
